...
class Vector:
    ...
    def __add__(self, other):
        return Vector([Decimal(x1) + Decimal(x2) for x1, x2 in zip(self.coords, other.coords)])
    ...
    def cross(self, vec2):
        if len(vec2.coords) != 3:
            return -1
        return Vector([self.coords[1] * vec2.coords[2] - self.coords[2] * vec2.coords[1],
                       -(self.coords[0] * vec2.coords[2] - self.coords[2] * vec2.coords[0]),
                       self.coords[0] * vec2.coords[1] - self.coords[1] * vec2.coords[0]])
...
class Converter:
    ...
    def __init__(self, pos=None, dir=None, coeff=None, m=None, b=None, eqs=None):
        ...
        #       1      2      4        8    16   32
        arr = locals()
        given = int(''.join([str(int(arr[s] is not None)) for s in attr][::-1]),
                    2)  # binary representation of given attributes
        print(given)
        if given == 32:
